...
$id_colaborador = isset($_POST['id_colaborador']) ? intval($_POST['id_colaborador']) : 0;
...
if ($id_colaborador === 0 || empty($nombre) || empty($apellido) || empty($email) || empty($genero) || empty($fecha_nacimiento)) {
    header("Location: ../vistas/editar_colaborador.php?id=$id_colaborador&error=Por favor, completa todos los campos obligatorios.");
    exit;
}

// Actualizar el colaborador
$sql = "UPDATE colaboradores 
        SET nombre = ?, apellido = ?, email = ?, genero = ?, fecha_nacimiento = ?, telefono = ?, departamento = ?, enfermedades_previas = ?
        WHERE id_colaborador = ?";
$stmt = $conexion->prepare($sql);
$stmt->bind_param("ssssssssi", $nombre, $apellido, $email, $genero, $fecha_nacimiento, $telefono, $departamento, $enfermedades_previas, $id_colaborador);
$stmt->execute();
$stmt->close();

// Insertar o actualizar datos biométricos
$sql = "INSERT INTO datos_biometricos (id_colaborador, peso, talla, perimetro_cintura, porcentaje_grasa, masa_muscular, presion_arterial_sistolica, presion_arterial_diastolica, frecuencia_cardiaca, glucosa_ayuno, colesterol_total, trigliceridos, fecha_medicion) 
        ...
        peso = ?, talla = ?, perimetro_cintura = ?, porcentaje_grasa = ?, masa_muscular = ?, 
        presion_arterial_sistolica = ?, presion_arterial_diastolica = ?, frecuencia_cardiaca = ?, 
        glucosa_ayuno = ?, colesterol_total = ?, trigliceridos = ?, fecha_medicion = NOW()";
$stmt = $conexion->prepare($sql);
$stmt->bind_param("idddddddddddiddddddddddd", 
    $id_colaborador, $peso, $talla, $perimetro_cintura, $porcentaje_grasa, $masa_muscular, 
    $presion_arterial_sistolica, $presion_arterial_diastolica, $frecuencia_cardiaca, $glucosa_ayuno, 
...
// Actualizar clases inscritas: primero eliminamos las existentes para el mes actual y luego insertamos las nuevas
$sql = "DELETE FROM inscripciones_clases WHERE id_colaborador = ? AND mes = ? AND anio = ?";
$stmt = $conexion->prepare($sql);
...
header("Location: ../vistas/ver_colaboradores.php?mensaje=Colaborador actualizado exitosamente");
exit;
